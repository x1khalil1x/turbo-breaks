...
@export var scene_type: String = "indoor"
...
func initialize_scene():
	...
	# 2. Register with MapManager if outdoor (indoor scenes don't register)
...
func validate_current_scene():
	...
	# Indoor-specific validations
	if scene_type == "indoor":
		# Check for exit doorways
		var doorways = get_tree().get_nodes_in_group("doorways")
		var exit_doorways = 0
		for doorway in doorways:
			if "exit" in doorway.doorway_id.to_lower():
				exit_doorways += 1
		
		if exit_doorways == 0:
			issues.append("Indoor scene should have at least one exit doorway")
